...
__global__ void SeedA(const siphash_keys &sipkeys, uint4 * __restrict__ buffer, int * __restrict__ indexes)
{
    ...
    for (int i = 0; i < loops; i++)
    {
      ...
      uint64_t node1;
      uint64_t node0 = dipnode(sipkeys, nonce, 0);
      ...
      if (counter == FLUSHA-1)
      {
        ...
        {
            buffer[((uint64_t)grp * maxOut + cnt + i) / TMPPERLL4] = *(uint4 *)(&tmp[row][i]);
        }
    ...
    uint32_t grp = row * NX + col;
      ...
      for (int i = 0; i < localIdx; i += TMPPERLL4)
      {
          ...
          buffer[((uint64_t)grp * maxOut + cnt) / TMPPERLL4] = *(uint4 *)(&tmp[row][i]);
      }
    }
}
...
__global__ void SeedB(const siphash_keys &sipkeys, const EdgeOut * __restrict__ source, uint4 * __restrict__ destination, const int * __restrict__ sourceIndexes, int * __restrict__ destinationIndexes) {
  ...
  for (int loop = 0; loop < loops; loop++)
  {
    ...
    if (counter == FLUSHB-1)
    {
      ...
      for (int i = 0; i < nflush; i += TMPPERLL4)
      {
          destination[((uint64_t)grp * maxOut + cnt + i) / TMPPERLL4] = *(uint4 *)(&tmp[col][i]);
      }
  ...
  for (int col = lid; col < NX; col += dim)
  {
    ...
    for (int i = 0; i < localIdx; i += TMPPERLL4)
    {
        ...
        destination[((uint64_t)grp * maxOut + cnt) / TMPPERLL4] = *(uint4 *)(&tmp[col][i]);
    }
  }
}
...
__device__ uint2 make_Edge(const uint64_t nonce, const uint2 dummy, const uint64_t node0, const uint64_t node1) {
   return make_uint2(node0, node1);
}

__device__ uint2 make_Edge(const uint2 edge, const uint2 dummy, const uint64_t node0, const uint64_t node1) {
   return edge;
}

__device__ uint64_t make_Edge(const uint64_t nonce, const uint64_t dummy, const uint64_t node0, const uint64_t node1)
{
   return nonce;
}

template <typename Edge> uint64_t __device__ endpoint(const siphash_keys &sipkeys, Edge e, int uorv);

__device__ uint64_t endpoint(const siphash_keys &sipkeys, uint32_t nonce, int uorv)
{
  return dipnode(sipkeys, nonce, uorv);
}

__device__ uint64_t endpoint(const siphash_keys &sipkeys, uint2 nodes, int uorv)
{
           ...
           int * __restrict__ destinationIndexes)
{
  ...
  for (int loop = 0; loop < loops; loop++)
  {
    ...
    if (lindex < edgesInBucket)
    {
      ...
      uint64_t node = endpoint(sipkeys, edge, round&1);
  ...
  for (int loop = 0; loop < loops; loop++)
  {
    ...
    if (lindex < edgesInBucket)
    {
      ...
      uint64_t node0 = endpoint(sipkeys, edge, round&1);

      if (Read2bCounter(ecounters, node0 & ZMASK))
      {
        uint64_t node1 = endpoint(sipkeys, edge, (round&1)^1);
...
typedef uint64_t proof[PROOFSIZE];
...
const static uint32_t MAXEDGES = 0x80000; // 0x20000 = 131072 aso
  ...
  for (uint32_t r = 0; r < range; r++)
  {
    ...
    for (unsigned s = 0; s < nsols; s++)
    {
      ...
      if (solutions != NULL)
      {
        ...
        for (unsigned int i = 0; i < PROOFSIZE; i++)
        {
...
int main(int argc, char **argv)
{
  ...
  fill_default_params(&params);
    ...
    while ((opt = getopt(argc, argv, optString)) != -1)
    {
        switch (opt)
        {
           ...
           case 's':
               ...
               fprintf(stdout, "Build date is: %s\n", bdate);
               //fprintf(stdout,"sizeof(uint2_t) = %d\n",sizeof(uint2_t));
               //fprintf(stdout,"sizeof(uint4_t) = %d\n",sizeof(uint4_t));
               fprintf(stdout,"sizeof(uint8_t) = %lu\n",sizeof(uint8_t));
               fprintf(stdout,"sizeof(uint16_t) = %lu\n",sizeof(uint16_t));
               fprintf(stdout,"sizeof(uint32_t) = %lu\n",sizeof(uint32_t));
               fprintf(stdout,"sizeof(uint64_t) = %ld\n",sizeof(uint64_t));
               fprintf(stdout,"sizeof(size_t) = %ld\n",sizeof(size_t));
               fprintf(stdout,"sizeof(ssize_t) = %ld\n",sizeof(ssize_t));
    ...
    if (will_debug)
    {
        ...
        fprintf(stdout, "Looking for 42-cycle on cuckoo%d(\"%s\",%zd", NODEBITS, header, nonce);
